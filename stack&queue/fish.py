...
and B represent N voracious fish in a river, ordered downstream along the flow of the river.
...
We assume that all the fish are flowing at the same speed. That is, fish moving
 in the same direction never meet. The goal is to calculate the number of fish that will stay alive.
            ...
            if len(stack) > 0:
                while len(stack) > 0:
                    if A[i] > stack[-1]:
                        ...
                        break
                if len(stack) == 0: survival += 1
            else:
                survival += 1

    return survival + len(stack)

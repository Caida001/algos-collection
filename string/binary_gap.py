...
representation 10000010001 and so its longest binary gap is of length 5. Given N = 32 the
...
def solution(N):
    ...
    for i in range(len(string)):
      if string[i] == '1': res.append(i)

    if len(res) <= 1: return 0

    return max(res[i+1] - res[i] - 1 for i in range(len(res) - 1))
